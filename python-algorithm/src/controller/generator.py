...
def generate_ed_matrix(ed_df):
    ...
    for col in ed_df.columns:
        if col not in ['doc_id', 'exp_id', 'weight']:
            raise ValueError(
                "ed_df columns should contains only exp_id, doc_id and weight")
...
def generate_tf(corpus, encoding='utf-8', token_pattern=r"(?u)\S\S+"):
    ...
    model = CountVectorizer(
        analyzer='word', encoding=encoding, token_pattern=token_pattern)
...
def generate_dtop_matrix(dp_matrix, topics, model_dict, top_n=1):
    ...
    for topic in topics:
        ...
        topic_phrase[topic] = [(sim_phrases[i], sim_vals[i])
                               for i in range(len(sim_phrases))]
...
def generate_pr_matrix(ed_matrix, dtopic_matrix, topic_vec, ed_graph, alpha=0.0):
    ...
    exp_pr_df, doc_pr_df = weight.calc_pr_weight(
        ed_matrix, dtopic_matrix, topic_vec, alpha)
